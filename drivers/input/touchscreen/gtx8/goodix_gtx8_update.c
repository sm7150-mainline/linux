...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	for (i = 0; i < fw_info->subsys_num; i++) {
		...
		fw_info->subsys[i].size = be32_to_cpup(
			(__be32 *)&firmware->data[info_offset + 1]);
		fw_info->subsys[i].flash_addr = be16_to_cpup(
			(__be16 *)&firmware->data[info_offset + 5]);
	...
	ts_debug("Firmware PID:GT%s", fw_info->fw_pid);
	ts_debug("Firmware VID:%02X%02X%02X%02x", fw_info->fw_vid[0],
			       ...
			       const struct firmware_info *fw_info)
{
	...
	if (ret) {
		ts_debug("failed get active pid");
		return -EINVAL;
	}
	...
	if (fw_ver.valid) {
		...
		if (ret == 0) {
			ts_debug("FW version is equal to the IC's");
			return 0;
		} else if (ret > 0) {
			   ...
			   struct firmware_data *fw_data)
{
	...
	ts_debug("Success select bank0, Set 0x%x -->0x01",
	...
	ts_debug("Success forbid bank0 accedd, set 0x%x -->0x00",
		 HW_REG_ACCESS_PATCH0);
	...
	if (ts_dev->ic_type == IC_TYPE_YELLOWSTONE)
		r = goodix_reg_write(ts_dev, HW_REG_CPU_RUN_FROM_YS, reg_val,
				     8);
	else
	...
	ts_debug("Success write [8]0x55 to HW_REG_CPU_RUN_FROM");
	...
	ts_err("ISP not work,0x%x=0x%x, 0x%x=0x%x", HW_REG_ISP_RUN_FLAG,
	       reg_val[0], HW_REG_ISP_RUN_FLAG + 1, reg_val[1]);
	return -EFAULT;
}
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (ts_dev->ic_type == IC_TYPE_YELLOWSTONE)
		ts_dev->hw_ops->write(ts_dev, HW_REG_CPU_RUN_FROM_YS, temp_buf,
				      8);
	else
	...
	ts_debug("firmware update, reset");
	...
	do {
		...
		if (r < 0)
			ts_debug("Failed to remove GIO hold flag, retry %d",
				retry);
		else
			break;
	} while (--retry);
	...
	for (retry = 0; retry < 20; retry++) {
		if (ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
			...
			r = goodix_reg_write(ts_dev, HW_REG_ESD_KEY_DIS,
					     reg_val, 1);
			if (r < 0) {
				ts_debug("failed dis esd key");
				continue;
			}
			reg_val[0] = 0x95;
			r = goodix_reg_write(ts_dev, HW_REG_ESD_KEY_EN, reg_val,
					     1);
			if (r < 0) {
				ts_debug("failed open esd key");
				continue;
			}
		}
		reg_val[0] = 0x24;
		r = goodix_reg_write_confirm(ts_dev, HW_REG_CPU_CTRL, reg_val,
					     1);
		if (r < 0) {
...
/**
 * goodix_format_fw_packet - format one flash packet
 * @pkt: target firmware packet
 * @flash_addr: flash address
 * @size: packet size
 * @data: packet data
 */
static int goodix_format_fw_packet(u8 *pkt, u32 flash_addr, u16 len,
				   const u8 *data)
{
	u16 checksum;
 ...
 * return：0 ok, <0 error
 */
static int goodix_send_fw_packet(struct goodix_ts_device *dev, u8 type, u8 *pkt,
				 u32 len)
{
	u8 reg_val[4];
	int r, i;
	...
	ts_debug("target fw subsys type:0x%x, len %d", type, len);
	...
	if (r < 0) {
		ts_err("Failed to clear flash flag");
		return r;
	}
	...
	ts_err("Wait for flash end timeout, 0x6022= %x %x", reg_val[0],
	       reg_val[1]);
	return -EAGAIN;
}
				  ...
				  struct fw_subsys_info *subsys)
{
	...
	while (total_size > 0) {
		...
		for (i = 0; i < 3; i++) {
			r = goodix_send_fw_packet(dev, subsys->type, fw_packet,
						  data_size + 6);
			if (!r)
				break;
		}
 ...
 *  and reset flags---
 * @fwu_ctrl: pointer to fw_update_ctrl structure
 * return: 0 ok, < 0 error
 */
				...
				struct fw_update_ctrl *fwu_ctrl)
{
	...
	for (i = 0; ts_dev->ic_type == IC_TYPE_YELLOWSTONE && i < 100; i++) {
		...
		if (!r)
			break;
		ts_debug("failed set GIO flag, r %d", r);
	}
	if (i >= 100)
...
static int goodix_flash_config(struct goodix_ts_device *ts_dev)
{
	...
	ts_debug("normal config length %d", ts_dev->normal_cfg.length);
	...
	ts_debug("try flash config");
	ret = goodix_send_fw_packet(ts_dev, FLASH_SUBSYS_TYPE_CONFIG, fw_packet,
				    ISP_MAX_BUFFERSIZE + 6);
	if (ret)
		ts_err("failed flash config, ret %d", ret);
	else
		ts_debug("success flash config with isp");
...
#define FW_UPDATE_RETRY 2
	...
	if (!(fwu_ctrl->mode & UPDATE_MODE_FORCE)) {
		...
		if (!r) {
			fwu_ctrl->status = UPSTA_ABORT;
			ts_debug("fw update skipped");
			goto err_check_update;
		}
	} else {
		ts_debug("force update mode");
	}
...
start_update:
	...
	if (GOODIX_FLASH_CONFIG_WITH_ISP &&
		...
		ts_debug("need flash config with isp");
					    ...
					    const char *buf, size_t count)
{
	...
	ts_debug("set update mode:0x%x", buf[0]);
	if (buf[0] == '1') {
	...
	if (!ret) {
		ts_debug("success start update work");
		return count;
	}
...
static ssize_t
goodix_sysfs_update_progress_show(struct goodix_ext_module *module, char *buf)
{
	struct fw_update_ctrl *fw_ctrl = module->priv_data;
	return scnprintf(buf, PAGE_SIZE, "%d\n", fw_ctrl->progress);
}

static ssize_t goodix_sysfs_update_result_show(struct goodix_ext_module *module,
					       char *buf)
{
	char *result = NULL;
	struct fw_update_ctrl *fw_ctrl = module->priv_data;
...
static ssize_t
goodix_sysfs_update_fwversion_show(struct goodix_ext_module *module, char *buf)
{
	struct goodix_ts_version fw_ver;
	struct fw_update_ctrl *fw_ctrl = module->priv_data;
	int r = 0;
...
static struct goodix_ext_attribute goodix_fwu_attrs[] = {
	...
	__EXTMOD_ATTR(progress, S_IRUGO, goodix_sysfs_update_progress_show,
		      NULL),
	__EXTMOD_ATTR(result, S_IRUGO, goodix_sysfs_update_result_show, NULL),
	__EXTMOD_ATTR(fwversion, S_IRUGO, goodix_sysfs_update_fwversion_show,
		      NULL),
				...
				struct goodix_ext_module *module)
{
	...
	for (i = 0; i < ARRAY_SIZE(goodix_fwu_attrs) && !ret; i++)
		ret = sysfs_create_file(&module->kobj,
...
int goodix_do_fw_update(int mode)
{
	...
	if (fwu_ctrl->mode & UPDATE_MODE_BLOCK) {
		...
		ts_debug("fw update return %d", ret);
		return ret;
	} else {
		/* create and run update thread */
		fwu_thrd = kthread_run(goodix_fw_update_thread, fwu_ctrl,
				       "goodix-fwu");
		...
		ts_debug("success create fw update thread");
		return 0;
	}
}
				 ...
				 struct goodix_ext_module *module)
{
	...
	if (goodix_fw_update_ctrl.initialized) {
		ts_debug("no need reinit");
		return ret;
	}

	if (!core_data || !ts_bdata || !core_data->ts_dev) {
		ts_err("core_data && ts_dev can't be null");
		return -ENODEV;
	}
				   ...
				   struct goodix_ext_module *module)
{
	...
	return fwu_ctrl->allow_resume ? EVT_HANDLED : EVT_CANCEL_RESUME;
}
			       ...
			       struct goodix_ext_module *module)
{
	...
	return fwu_ctrl->allow_irq ? EVT_HANDLED : EVT_CANCEL_IRQEVT;
}
...
{
	ts_debug("goodix_fwupdate_module_ini IN");
