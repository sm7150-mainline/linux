				  ...
				  struct goodix_cfg_package *cfg_pkg)
{
	...
	ts_cfg->length =
		cfg_pkg->pkg_len - TS_PKG_CONST_INFO_LEN - TS_PKG_REG_INFO_LEN;
				  ...
				  struct goodix_cfg_bin *cfg_bin)
{
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		/*get ic type*/
		if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "normandy",
			     strlen("normandy"))) {
			...
			ts_err("unknown ic type of cfg_bin:%s",
			       cfg_bin->cfg_pkgs[i].cnst_info.ic_type);
			continue;
		}
		...
		if (r < 0) {
			ts_err("failed get sensor of pkg:%d, reg:0x%02x", i,
			       addr);
		...
		if (r < 0) {
			ts_err("failed read pid pkg:%d, pid reg:0x%02x", i,
			       addr);
			goto get_default_pkg;
		}
		if (strncmp(temp_pid, cfg_bin->cfg_pkgs[i].cnst_info.hw_pid,
			    read_len)) {
		...
		if (cfg_bin->cfg_pkgs[i].cnst_info.cfg_type == TS_NORMAL_CFG ||
		    cfg_bin->cfg_pkgs[i].cnst_info.cfg_type ==
			    TS_HIGH_SENSE_CFG) {
			r = goodix_extract_cfg_pkg(ts_dev,
						   &cfg_bin->cfg_pkgs[i]);
			if (!r) {
...
static int goodix_later_init_thread(void *data)
{
	...
	if (ts_dev->cfg_bin_state == CFG_BIN_STATE_TEMP) {
		...
		ret = goodix_do_fw_update(
			...
			UPDATE_MODE_FLASH_CFG | UPDATE_MODE_SRC_REQUEST);
		if (ret) {
