...
static int async_read(struct goodix_tools_dev *dev, void __user *arg)
{
	...
	if (!hw_ops->read_trans(ts_dev, reg_addr, databuf, length)) {
		if (copy_to_user((u8 *)arg + I2C_MSG_HEAD_LEN, databuf,
				 length)) {
			ret = -EFAULT;
...
static int read_config_data(struct goodix_ts_device *ts_dev, void __user *arg)
{
	...
	if (!reg_addr) {
		...
		ret = ts_dev->hw_ops->read_trans(ts_dev, reg_addr, tmp_buf,
						 length);
		if (!ret)
			ret = length;
	}
 ...
 * @cmd: Ioctl operation command
 * @arg: Command data
 * Returns >=0 - succeed, else failed
 */
static long goodix_tools_ioctl(struct file *file, unsigned int cmd,
			       unsigned long arg)
{
	int ret = 0;
	struct goodix_tools_dev *dev = file->private_data;
	...
	if (_IOC_NR(cmd) > GOODIX_TS_IOC_MAXNR) {
		ts_err("Bad cmd num:%d > %d", _IOC_NR(cmd),
		       GOODIX_TS_IOC_MAXNR);
		return -ENOTTY;
	}
